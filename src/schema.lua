  ...
  fields = {
    ...
    { config = {
        ...
        fields = {
          ...
          { echo = { description = "When set, the plugin will echo a copy of the request back to the client. The main usecase for this is debugging. It can be combined with `trigger_key` and `trigger_value` in order to debug requests on live systems without disturbing real traffic.", type = "boolean", required = true, default = false }, },
          { trigger_key = { description = "when set, the plugin allows requests which contains key, value pair in headers or in query.", type = "string" }, },
          { trigger_value = { description = "see `trigger_key`", type = "string" }, }
        },
