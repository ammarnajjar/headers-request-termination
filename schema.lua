  ...
  fields = {
    ...
    { config = {
        ...
        custom_validator = function(config)
          if (is_present(config.trigger_key) and !(is_present(config.trigger_value)) or
		  !is_present(config.trigger_key) and (is_present(config.trigger_value)) )
		  then
            return nil, "trigger needs both key and a value"
          end
